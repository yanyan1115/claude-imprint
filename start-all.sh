...
if is_running .pid-http; then
    ...
    imprint-memory --http > logs/http.log 2>&1 &
    echo $! > .pid-http
    sleep 2
...
echo "   Dashboard: python3 packages/imprint_dashboard/dashboard.py"
echo "   Heartbeat: cd packages/imprint_heartbeat && ./start.sh"
