...
- `imprint-memory`: 记忆核心包、SQLite 数据库、embedding、HTTP OAuth、message bus、压缩模块。
...
| `imprint-memory --http` OAuth | `~/.imprint-oauth.json` 优先，其次环境变量 | 如果凭证文件存在，`OAUTH_*` 环境变量会被忽略。 |
...
`imprint-memory` 的数据库层在 `imprint_memory/db.py` 中定义：
...
原因：`imprint-memory` 核心包支持 `IMPRINT_DB`，但 `claude-imprint` 的 Dashboard、`update_claude_md.py`、Heartbeat 等代码直接读取 `$IMPRINT_DATA_DIR/memory.db`。如果只改 `IMPRINT_DB`，核心服务和 Dashboard 可能看见不同数据库。
...
sudo systemctl restart imprint-memory@$USER imprint-dashboard@$USER imprint-heartbeat@$USER
```
...
| 变量 | 默认值 | 必填 | 使用位置 | 作用 |
|---|---:|---|---|---|
| `IMPRINT_DATA_DIR` | `~/.imprint` | 否 | `imprint-memory`、Dashboard、Heartbeat、`update_claude_md.py`、hooks | 数据根目录。systemd 模板默认 `/home/%i/.imprint`，hooks 当前强制为项目根目录。 |
| `IMPRINT_DB` | `$IMPRINT_DATA_DIR/memory.db` | 否 | `imprint-memory` 核心包 | 覆盖 SQLite 路径。full-stack 场景不建议单独设置。 |
| `TZ_OFFSET` | `0` | 否 | `imprint-memory`、Dashboard、Heartbeat、`update_claude_md.py` | 固定 UTC 小时偏移，例如 `8`、`-4`。代码不处理 IANA 时区或 DST。必须是整数，否则进程导入时会报错。 |

### Embedding 与检索

| 变量 | 默认值 | 必填 | 使用位置 | 作用 |
|---|---:|---|---|---|
| `EMBED_PROVIDER` | `ollama` | 否 | `imprint_memory/memory_manager.py` | embedding provider。只有值为 `openai` 时走 OpenAI-compatible API，其他值都会走 Ollama 分支。 |
...
| 变量 | 默认值 | 必填 | 使用位置 | 作用 |
|---|---:|---|---|---|
| `OAUTH_CLIENT_ID` | 空字符串 | HTTP OAuth 环境变量模式下需要 | `imprint_memory/server.py` | OAuth client id。 |
| `OAUTH_CLIENT_SECRET` | 空字符串 | HTTP OAuth 环境变量模式下需要 | `imprint_memory/server.py` | OAuth client secret。 |
| `OAUTH_ACCESS_TOKEN` | 空字符串 | 对外暴露 HTTP 服务时需要 | `imprint_memory/server.py` | Bearer token。远端请求需要 `Authorization: Bearer <token>`。 |
...
| 变量 | 默认值 | 必填 | 使用位置 | 作用 |
|---|---:|---|---|---|
| `COMPRESS_MODEL` | `qwen3:8b` 或 `goekdenizguelmez/JOSIEFIED-Qwen3:8b` | 否 | `imprint_memory/compress.py`、`hooks/post_response_processor.py` | 压缩/摘要使用的 Ollama 模型。核心压缩模块默认 `qwen3:8b`，post-response hook 摘要默认 JOSIEFIED 模型。 |
| `COMPRESS_KEEP` | `30` | 否 | `imprint_memory/compress.py` | 压缩上下文时保留的最近行数。 |
| `COMPRESS_THRESHOLD` | `50` | 否 | `imprint_memory/compress.py` | 触发压缩的 message line 数。 |
| `MESSAGE_BUS_LIMIT` | `40` | 否 | `imprint_memory/bus.py` | message bus 保留的最近消息数，写入新消息时自动裁剪。 |
...
| `PATH` | 取决于 shell/systemd | 视部署而定 | `deploy/imprint-telegram@.service`、Heartbeat 子进程 | 确保 `claude`、`bun`、`imprint-memory` 等命令可执行。Telegram systemd 模板显式设置了 `/home/%i/.local/bin:/usr/local/bin:/usr/bin:/bin`。 |
| `CLAUDECODE` | 继承环境 | 否 | `imprint_memory/tasks.py`、Heartbeat 子进程 | 子进程启动 Claude CLI 前会删除该变量，避免嵌套环境污染。不是用户配置项。 |
...
| 文件或命令 | 默认位置 | 用途 |
|---|---|---|
| `~/.imprint-oauth.json` | 当前用户 home | `imprint-memory --http` OAuth 凭证。优先级高于 `OAUTH_*` 环境变量。 |
...
| `.mcp.json` / `cron-mcp*.json` | 项目根目录 | MCP server 命令配置。示例只配置 `imprint-memory`，`cron-mcp-full.json` 额外包含 Telegram 和 utils MCP。 |
...
| Unit | 默认环境变量 |
|---|---|
| `imprint-memory@.service` | `IMPRINT_DATA_DIR=/home/%i/.imprint`、`TZ_OFFSET=0`，并注释了 `EMBED_PROVIDER=openai`、`OPENAI_API_KEY=sk-...` |
...
- `imprint-memory` 可执行文件在 `/home/%i/.local/bin/imprint-memory`。
