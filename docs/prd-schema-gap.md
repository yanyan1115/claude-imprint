...
| Memory write extraction prompt | Core write path supports emotional fields, but this repo does not own the external LLM extraction prompt. | Partial | PRD requires stable third-person extraction and empty-result retry. Current `claude-imprint` cannot fully verify external core prompt behavior. | Add regression docs/tests in `imprint-memory` or pin expected prompt behavior in API docs. |
