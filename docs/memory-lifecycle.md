...
本文档描述 `memo-clover` 当前实现中的记忆生命周期：写入、去重、历史化、检索、访问激活、情感衰减、归档、主动浮现和物理删除。核心代码位于：

- `memo_clover/db.py`
- `memo_clover/memory_manager.py`
- `memo_clover/server.py`
...
| server background thread | `memo-clover` 启动后 300 秒第一次运行，之后每 86400 秒运行一次，`dry_run=False`。 |

后台线程在 `memo_clover/server.py main()` 中启动。stdio 模式和 HTTP 模式都会启动该线程。
