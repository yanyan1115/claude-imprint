...
| Memory | `#memory-section` | `/api/memories`, `/api/decay-status`, `/api/search-status` | 搜索、编辑、删除记忆，并展示情感/衰减元数据和检索模式。 |
...
| `GET /api/search-status` | 返回当前检索模式：向量检索或纯文本降级。 |
...
| `fetchSearchStatus()` | 30 秒 | Memory 搜索栏旁的检索模式状态。 |
...
- 返回 `meta.total`、`meta.has_more`、`meta.page` 和 `meta.limit`。无状态过滤时，`meta.total` 是 SQLite 的真实总数，不再用“当前页 + 是否还有更多”的近似值。
- 返回 `meta.has_more=true` 时，前端显示加载更多按钮；点击后把 `page` 加 1，并把下一页结果追加到当前列表。
- 加载更多期间，按钮会显示 `Loading...` / `加载中...`，并进入 disabled 状态，避免网络卡顿时重复点击造成重复渲染。全部加载完成后，底部保留 `No more memories` / `没有更多了` 提示。
...
| `保护` / `protected` | `protected` | 只显示 pinned、core_profile 或 `decay_rate = 0` 的记忆。 |
| `主动浮现` / `surfacing` | `surfacing` | 只显示未解决且 arousal 较高的记忆。 |
| `已解决` / `resolved` | `resolved` | 只显示 `resolved` 为真的记忆。 |
| `已归档` / `archived` | `archived` | 只显示 `archived`、`is_archived` 或状态归档的记忆。 |
| `低分` / `low score` | `low_score` | 只显示 `decay_score < 0.3` 或当前衰减状态为 low_score 的记忆。 |
| `衰减中` / `decaying` | `decaying` | 只显示当前衰减状态为 decaying 的记忆。 |

后端会尽量把 `status` 条件下推到 SQLite 查询中；如果当前 schema 缺少某些列，则继续用 Dashboard 的派生 `decay_status` 做兼容过滤。

被选中的状态标签会添加 active 样式，高亮当前过滤视图。再次点击当前标签，或点击 `总数` / `total`，会取消选中并恢复显示全部。每次切换标签都会强制重置 `page = 1`、清空当前列表并重新请求，避免带着旧分页进入新过滤条件。搜索框输入变化、编辑或删除记忆后，也会按当前状态过滤重新拉取列表。
...
所有记忆编辑失败都会写入后端 logger。前端不会直接展示 SQLite 底层错误，而是用 Toast 显示 `Save failed. Please try again.` / `保存失败，请稍后重试。`，底层异常保留在浏览器 console 和 Dashboard 日志中。
...
Memory 区域标题右侧有一个小型 pill，显示当前检索模式：

| 状态 | 颜色 | 含义 |
|---|---|---|
| `Vector Search` / `向量检索` | 绿色 | Dashboard 探测到 embedding provider 正常响应，MemoClover 可以使用语义向量召回。 |
| `Text-only (Fallback)` / `纯文本检索（降级）` | 黄色 | embedding provider 未响应、未配置或返回异常；MemoClover 会跳过向量通道，继续使用 FTS5 + LIKE 文本检索。 |
| `Search status unavailable` / `检索状态不可用` | 灰色 | Dashboard 无法读取 `/api/search-status`，通常是 Dashboard API 异常或浏览器请求失败。 |

后端接口是：

```text
GET /api/search-status
GET /api/search-status?force=true
```
