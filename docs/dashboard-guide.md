...
| Key | 命令 | 类型 | 状态检测 |
|---|---|---|---|
| `memory_http` | `imprint-memory --http` | background | 优先检测 `:8000` 端口，其次 PID 文件。 |
...
1. `content`、`category`、`importance` 通过 `imprint_memory.memory_manager.update_memory()` 更新；如果更新内容，核心包会刷新 embedding。
